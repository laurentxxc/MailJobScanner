#!/usr/bin/env python3
"""Core job-analysis orchestration pipeline.

Processes an email into job proposals, fetches job descriptions, runs LLM
matching against the resume and expectations, computes commute info, and
persists results. Shared by the CLI (main.py), the Gmail fetcher
(gmail_fetcher.py) and the dashboard (dashboard.py).
"""
import logging
import time
import urllib.parse
...
from config import load_linkedin_cookies
from core.llm.commute import get_commute_info
from core.llm.llm_client import LlmClient
from core.parsing.email_parser import parse_eml
